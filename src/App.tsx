...
function Site() {
  ...
  return <div className="site-shell"><div className="sticky-group"><Ticker /><Header onMenu={() => setMenuOpen(!menuOpen)} menuOpen={menuOpen} /></div><Routes><Route path="/admin" element={<Admin />} /><Route path="/" element={<Home />} /><Route path="/membership" element={<MembershipPage />} /><Route path="/blood-donation" element={<FormPage type="bloodDonation" />} /><Route path="/overseas-registration" element={<FormPage type="overseasRegistration" />} /><Route path="/cabinet" element={<Cabinet />} /><Route path="/:type" element={<CollectionPage />} /></Routes><Footer /></div>;
}
...
function CandidateCard({ item, onVoted }: { item: RecordItem; onVoted?: () => void }) {
  ...
  return <article className={`candidate-card ${voted ? 'candidate-card-voted' : ''}`}>
    {item.photoUrl ? <img src={item.photoUrl} alt={text(item, 'fullName') || 'Candidate'} /> : <div className="candidate-photo-fallback">{(text(item, 'fullName') || 'C').slice(0,2).toUpperCase()}</div>}
    <div className="candidate-card-body"><span className="soft-tag">{text(item, 'position') || 'Candidate'}</span><h2>{text(item, 'fullName') || 'Community candidate'}</h2>
      <p>{text(item, 'bio', 'details', 'manifesto') || 'Candidate details will be shared here.'}</p>
      <div className="candidate-meta">{text(item, 'phone') && <span>Phone: {text(item, 'phone')}</span>}{text(item, 'email') && <span>Email: {text(item, 'email')}</span>}{text(item, 'cnic') && <span>CNIC / ID: {text(item, 'cnic')}</span>}<span>Voting: {item.votingStart || 'To be announced'} → {item.votingEnd || 'To be announced'}</span></div>
      <button type="button" className={`candidate-vote-button ${voted ? 'voted' : ''}`} onClick={vote} disabled={voted || saving}>{voted ? 'Vote submitted' : saving ? 'Submitting…' : `Vote for ${text(item, 'fullName') || 'candidate'}`}</button>
    </div>
  </article>;
}

function CollectionPage() { const { type = '' } = useParams(); const section = sections.find((s) => s.path.slice(1) === type) || sections[2]; const [items, setItems] = useState<RecordItem[]>([]); useEffect(() => { readCollection(section.key).then(setItems); }, [section.key]); const records: RecordItem[] = section.key === 'cabinet' ? (items.length ? items : fallbackCabinet) : items; const competitions = records.reduce((groups: Record<string, RecordItem[]>, item) => { const position = text(item, 'position', 'role') || 'Community position'; (groups[position] ||= []).push(item); return groups; }, {}); return <main className="page"><div className="container page-head"><Link to="/" className="back-link"><ChevronLeft size={16} /> Back home</Link><div className="eyebrow">Zwanan Jawkhela · Community services</div><h1>{section.title}</h1><p>{section.description}</p></div>{section.key === 'cabinet' ? <CabinetCarousel members={records} /> : section.key === 'candidates' ? <div className="container elections-list">{records.length ? Object.entries(competitions).map(([position, candidates]) => <section className="election-competition" key={position}><div className="election-competition-heading"><div><span className="eyebrow">Position competition</span><h2>{position}</h2><p>Choose one candidate for this position.</p></div><span className="competition-count">{candidates.length} candidates</span></div><div className="candidate-comparison">{candidates.map((item, index) => <React.Fragment key={item.id || index}>{index > 0 && <div className="vs-badge" aria-hidden="true">VS</div>}<CandidateCard item={item} /></React.Fragment>)}</div></section>) : <EmptyState message="No candidates have been added yet." />}</div> : <div className="container cards-list">{records.length ? records.map((item, i) => <div key={item.id || i}><DataCard item={item} section={section.title} /></div>) : <EmptyState message="This space is ready for community updates. Check back soon or contact the cabinet to contribute information." />}</div>}</main>; }
