...
export function ImageStreamHero({
  ...
  const right = `cabinet-stream-right-${id}`;
  const left = `cabinet-stream-left-${id}`;
  const card = `cabinet-stream-card-${id}`;
  ...
  return (
    <div className={`relative overflow-hidden ${className || ''}`} {...props} style={{ containerType: 'inline-size', ...style }}>
      <style>{css}</style>
        ...
        <div className="absolute inset-0" style={{ transformStyle: 'preserve-3d' }}>
          {[right, left].map((name) => Array.from({ length: cards }, (_, i) => {
            ...
            return (
              <div
                key={`${name}-${i}`}
                className={`${card} absolute overflow-hidden border border-white/30 bg-white/10 shadow-2xl backdrop-blur-sm`}
                style={{
                  left: '50%',
                  top: `${axis}%`,
              ...
              >
                {image && <img src={image.src} alt={image.alt || ''} loading="lazy" decoding="async" className="h-full w-full object-cover" draggable={false} />}
              </div>
            );
          }))}
        </div>
